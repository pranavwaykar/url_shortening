...
interface ButtonProps {
  ...
  onClick?: React.MouseEvent<HTMLElement> | null;
}
...
export default function Button({
  ...
  const styles: { [key: string]: string } = {
    ...
    copy: `${base} py-6 flex items-center justify-center rounded-md font-bold md:min-w-fit h-0 px-0 min-w-full text-sm`,
  };

  return (
    <button className={styles[type]} onClick={() => {}}>
      {children}
    </button>
  );
}
